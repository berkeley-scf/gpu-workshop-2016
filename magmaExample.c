...
// compile as:
// gcc magmaExample.c -O3 -DADD_ -fopenmp  -DHAVE_CUBLAS -I/usr/local/cuda/include -I/usr/local/magma/include -L/usr/local/cuda/lib64 -L/usr/local/magma/lib -lmagma  -llapack -lblas -lcublas -lcudart -o magmaExample
...
int main( int argc, char **argv ) {
  ...
  for( size = 512; size <= 8192; size*=4 ) {
    ...
    magma_dpotrf_gpu(MagmaLower, size, dA, size, &info );
    ...
    magma_dpotrf(MagmaLower, size, A, size, &info );
